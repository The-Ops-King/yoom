...
## Amendment (Fable review, 2026-09-02): clipboard needs the slug before the upload

`navigator.clipboard.writeText` only works within a user gesture's transient activation (~5 s in Chrome). Task 20 as written copies after `uploadRecording()` resolves, which for any real recording is far later than that, so `copied` would always be false and the toast would never show. Replace Task 20's approach with **slug reservation**:

- **`src/app/api/upload/route.ts`** (Phase 1 file, small change): generate `slug = newSlug()` and return `{ sessionUri, slug }` alongside the session (no DB write yet; the slug is only a suggestion).
- **`src/app/api/upload/complete/route.ts`**: accept optional `slug` in the body; validate with `SLUG_RE`; use it for the first insert attempt and fall back to `newSlug()` on a unique violation (the existing retry loop). Response unchanged.
- **`src/lib/recording/upload.ts`**: `uploadRecording` gains an `onSlug?: (url: string) => void` callback invoked right after `/api/upload` returns (with `shareUrl(slug)` built client-side from `NEXT_PUBLIC_SHARE_BASE_URL` — `shareUrl()` from `@/lib/share` is safe in client code since `env.ts` reads `NEXT_PUBLIC_*` literally) and passes `slug` to `/api/upload/complete`. Update `upload.test.ts` (assert `onSlug` fires before the first chunk PUT and that `complete` receives `slug`).
- **`src/lib/recording/use-recorder.ts` (Task 20)**: in `upload()`, the `/api/upload` round-trip completes well inside the click's activation window, so copy there: `onSlug: (url) => { navigator.clipboard.writeText(url).then(() => { copiedRef.current = true }).catch(() => {}) }`. After `UPLOAD_DONE`, `router.push(\`/library/${result.id}${copiedRef.current ? "?new=1" : ""}\`)`. If the final slug differs from the reserved one (collision fallback), the copied link would be wrong — so `/api/upload/complete` must **prefer the reserved slug** and only fall back on a genuine collision, and the detail page's toast copy should read "Uploaded. Share link copied." plus show the link so a rare mismatch is visible. Add a `console.warn` in the hook when `result.slug !== reservedSlug`.
- **Tests**: `upload.test.ts` for `onSlug` ordering; a route-level note in Task 22's manual checklist: "click Upload, immediately paste somewhere → the pasted link matches the detail page's link."

Also (lint): `EditPlayer` (Task 13) assigns `editsRef.current = edits` during render, which trips `react-hooks/refs` in this project's ESLint config — move it into a `useEffect`.
